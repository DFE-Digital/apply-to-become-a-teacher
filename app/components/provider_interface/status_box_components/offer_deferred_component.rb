      ...
      def add_change_links_to(rows)
        rows.map do |row|
          ...
          when 'Training provider'
            row.merge(action: { href: action_path(:provider) })
          when 'Course'
            row.merge(action: { href: action_path(:course) })
          when 'Full time or part time'
            row.merge(action: { href: action_path(:study_mode), visually_hidden_text: 'if full time or part time' })
          when 'Location'
            row.merge(action: { href: action_path(:course_option) })
          else
            row
          end
        end
